classdef EncoderOnlyMismatch < handle
    % EncoderOnly class for handling trials in  which the treadmill velocity alone determines the structure of the trial and there is a "mismatch" event at some point along the trial.
    %
    ...
    properties
        ...
        back_limit % Backward limit beyond which the trial is stopped.
        forward_limit % Forward limit beyond which the trial is stopped and reward given.
        ...
        integrate_using = 'pc' % Indicates whether to use the position on the pc (Position class) or on the Teensy to determine position. If using 'teensy' the trial will wait for a trigger from the Teensy before stopping. Otherwise the `position` varialble in the Position class is used to determine trial end. Can be 'teensy' or 'pc'.
    end
    ...
    methods
        ...
        function final_position = run(obj)
            ...
            % 10. Pause, simulate amount of time in :class:`rc.prot.Coupled` for calibrating the stage.
            ...
            % 16. Now wait for the position in Position class to reach the :attr:`switch_pos` position. When the point is reached send a trigger to the Teensy to increase or decrease the gain.
            % 17. After :attr:`mismatch_duration` seconds, send the trigger low to tell the Teensy to set the gain to 1.
            % 18. Now wait for end of trial. If :attr:`integrate_using` is 'teensy' it will wait for a trigger from the Teensy. If set to 'pc' it will wait for the position variable in the Position (`position` property in RC2Controller object) to read the trial bounds.
            %
            ...
            % 23. Pause, simulate amount of time after a :class:`rc.prot.Coupled` trial for the stage to move back.
            % 24. If this is being run as a single trial, stop NIDAQ acqusition and sound (do not do this if being run as a sequence, as the ProtocolSequence object will handle acquisition and sound).
            %
            % If error occurs:
            ...
            % the :meth:`stop` method is run (e.g. when the stage is moving to its
            % position).
