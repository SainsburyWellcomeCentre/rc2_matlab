classdef VisualContrast < handle
    ...
    methods
        function obj = VisualContrast(ctl, config, view)
            obj.ctl = ctl; 
            obj.config = config;
...
%                 end
                ...
                % *latency VisStim trigger to initiate whitenoise
                obj.ctl.vis_stim_latency.pulse(obj.sequence{i}.vis.latency*1000);
%                 pause(0.5);
                while obj.ctl.ni.do.is_running
                    pause(0.005);
                end
...
%                 if obj.sequence{i}.stage.enable_motion      % if enable stage rotation
                    ...
                    % visual stimuli initiation. visual stimuli starts at the start of trial triggered by VisStim trigger listening to NIDAQ AO(visual_stimuli function)
                    
                    while obj.ctl.ni.ao.task.IsRunning  % check to see AO is still running
                        if ~lickdetect_trigger
                            
                            % start lick detection
                            if obj.ctl.ni.ao.task.ScansOutputByHardware >= obj.ctl.lick_detector.delay*obj.config.nidaq.rate  % after delay, send lick_detector trigger to start lick detection
...
%                 end
                ...
                if contains(obj.sequence{i}.trial.stimulus_type, 'plus')
                ...
                elseif contains(obj.sequence{i}.trial.stimulus_type, 'minus')
