classdef RotationHabituation < handle
...
%         is_correct = []
    ...
    methods
        function obj = RotationHabituation(ctl, config, view)
            obj.ctl = ctl; 
            obj.config = config;
...
%             obj.ctl.play_sound(); 
            ...
            for i = 1 : length(obj.sequence)    % trial start
                ...
                if ~ true %obj.sequence{i}.stage.central.enable
                    target_axes(1) = NaN;
                end
                if ~ false %obj.sequence{i}.stage.outer.enable
                    target_axes(2) = NaN;
                end
...
%                     obj.ctl.vis_stim.on();
                    ...
                    while obj.ctl.ni.ao.task.ScansOutputByHardware >= (j-1)*refresher_delay*obj.config.nidaq.rate & obj.ctl.ni.ao.task.ScansOutputByHardware < j*refresher_delay*obj.config.nidaq.rate
                        pause(0.005);
                        if obj.ctl.ni.ao.task.ScansOutputByHardware == j*refresher_delay*obj.config.nidaq.rate
                            break
                        end
                    end
                end
