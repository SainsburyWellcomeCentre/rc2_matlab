function config = config_EnsembleTest()
...
config.nidaq.ai.channel_names           = {'stage_central', 'stage_outer', 'waveform_0',  'waveform_1'};  % nominal channel names (for reference)
config.nidaq.ai.channel_id              = [0, 1, 16, 18];     
config.nidaq.ai.offset                  = [0.004, 0.005, 0, 0];
...
config.stage.max_limits                 = [-1000 1000];    
...
config.ensemble.ai_offset               = [-0.00095 0.5];   % The offset applied to the Ensemble analog input (mV).
...
config.lick_detect.trigger_channel      = 6;    % index of channel in "config.nidaq.ai.channel_names" not analog input channel ID  
config.lick_detect.lick_channel         = 8;    % index of channel in "config.nidaq.ai.channel_names" not analog input channel ID  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
config.plotting                         = plotting_config_EnsembleTest();  % get plotting configuration from a separate file
